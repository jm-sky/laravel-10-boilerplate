    ...
    <div class="w-full md:w-2/3 flex flex-col items-center text-center">
        ...
        <form class="flex w-full justify-center items-end" action="{{ route('listings.index') }}" method="get">
            <div class="relateive mr-4 w-full lg:w-1/2 text-left">
                <input type="text" name="s" value="{{ request()->s }}" class="w-full bg-gray-100 bg-opacity-50 rounded focus:ring-2 focus:ring-primary-200 focus:bg-transparent" />
            </div>
            <button class="inline-flex text-white bg-primary-500 border-0 py-2 px-6 focus:outline-none hover:bg-primary-600 rounded text-lg">Search</button>
        </form>
